...
public class Tetrimino
	{	
		...
		//Retruns the last index of fourBlocks[]
		...
		/**
		 * Retruns sideBlocksRight<Integer> after finding
		...
		public ArrayList<Integer> sideBlocksRight()
		{
			...
			for (int i = 1; i < 4; i++)	//for-loop that goes through each block starting from the 2nd block
			{
				...
				{
					sideBlocksRight.add(mostRight);	//adds the block deamed mostRight from the row add the current block
		...
		/**
		 * Retruns sideBlocksLeft<Integer> after finding
		...
		public ArrayList<Integer> sideBlocksLeft()
		{
			...
			for (int i = 2; i > -1; i--)	//for-loop that goes through each block starting from the 3rd block
			{
				...
				{
					sideBlocksLeft.add(mostLeft);	//adds the block deamed mostLeft from the row below the current block
					currX = fourBlocks[i]/10;	//changes currX to the row of the current block
